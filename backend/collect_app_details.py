...
def collect_app_store_details(limit: int):
...
def collect_app_store_reviews(limit: int):
    """App Store 리뷰를 수집합니다."""
...
def collect_play_store_details(limit: int):
...
def collect_play_store_reviews(limit: int):
...
def main():
    ...
    parser.add_argument('--limit', type=int, default=1000, help='Max apps to process (default: 1000)')
    ...
    print(f"{'=' * 60}")
    print(f"Limit: {args.limit} apps per task")
    print()
