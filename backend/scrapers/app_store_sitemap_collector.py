...
from scrapers.sitemap_utils import (
    ...
    filter_best_country_per_language, log_sitemap_step_end
)
from database.sitemap_apps_db import (
    get_sitemap_file_hash,
    update_sitemap_file,
    upsert_app_localizations_batch,
    SitemapDbSession,
)
...
class AppStoreSitemapCollector:
    ...
    def log(self, message: str):
        if self.verbose:
            self.logger.info(f"[AppStore] {message}")
    ...
    def process_sitemap_file(self, sitemap_url: str, db_session: Optional[SitemapDbSession] = None) -> int:
        ...
        def process_with_session(session: SitemapDbSession) -> int:
            # 기존 해시와 비교
            existing_hash = get_sitemap_file_hash(sitemap_url, session=session)
            ...
            new_count = upsert_app_localizations_batch(localizations, session=session)

            # sitemap 파일 정보 업데이트
            update_sitemap_file(PLATFORM, sitemap_url, content_hash, len(localizations), session=session)
            ...
            self.log(
                f"Processed {filename}: {len(raw_localizations)} raw -> "
                f"{len(localizations)} filtered ({new_count} new)"
            )
            log_sitemap_step_end(self.logger, filename, start_perf, "OK")
            return new_count

        if db_session:
            return process_with_session(db_session)
    ...
    def collect_all(self) -> Dict[str, Any]:
        ...
        with SitemapDbSession() as session:
            for i, sitemap_url in enumerate(all_sitemap_urls, 1):
                ...
                self.process_sitemap_file(sitemap_url, db_session=session)
